...
if (x[3] == undefined) {
    document.getElementById("second_page_return_date").style.display = "none";
...
function HitFlightDearch(AccessContainer, statusOfReturn, starthere, endhere, thisdate) {
    ...
    // handle round way
    // agar user ne round way select kiya h to

    if (statusOfReturn === true) {
        temp = starthere;   //swaping the way
        starthere = endhere;
        endhere = temp;
        thisdate = x[3]; // asigning return date 
    }
    ...
    console.log(x[3], "x[3]");
    if (x[3] == undefined) {
        document.getElementById("second_page_return_date").display = "none";
        console.log("inside condition");
        console.log(x[3], "asdfghjkasdfghjk");
        document.getElementById("resultdiv1").style.gridArea = "1/1/7/12";
        document.getElementById("resultdiv2").style.display = "none";
        ...
        let check = new Promise(async (response, rejected) => {
            try {
                ...
                for (let i = 0; i < c.length; i++) {
                    ...
                    document.getElementById(AccessContainer).appendChild(divtag);
                }
...
let statusOfReturn = false;
HitFlightDearch("resultdiv1", statusOfReturn, x[0], x[1], x[2]);
if (x[3] != null || x[3] != undefined) {
    statusOfReturn = true;
    HitFlightDearch("resultdiv2", statusOfReturn, x[0], x[1], x[2]);
}
// x[3] != null || x[3] != undefined
// if (false) {
...
function Handle_Second_Page_Inpiut(inputtag, from_HTML_ListContainer) {
    let val = document.getElementById(inputtag).value;
    console.log(val);
    let listofAirports = airportskeys.filter((element) => element.indexOf(val) == 0 && element.length > val.length);
    document.getElementById(from_HTML_ListContainer).innerHTML = "";
    let udatedList = listofAirports.map((element) => {
        let tag = document.createElement("option");
        tag.innerText = element;
        document.getElementById(from_HTML_ListContainer).appendChild(tag);
        console.log(tag);
        return tag;
    })
...
function Second_Page_Search() {
    let first = document.getElementById("firstinput").value;
    let second = document.getElementById("secondinput").value;
    let first_Date = document.getElementById("firstdate").value;

    // date humko 12/23/2022 k format me mil raha h but humko chahiye 2022-12-23 
    statusOfReturn = false;
    first_Date = first_Date.split('/');
    let final_first_date = first_Date[2] + "-" + first_Date[1] + "-" + first_Date[0];
    x[3] = undefined;
    HitFlightDearch("resultdiv1", statusOfReturn, first, second, final_first_date);
}
